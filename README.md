...
- **일간 자동 전송 (스케줄러)**: 매주 평일(월~금) **아침 6시 정각(KST)**에 당일 요일에 해당하는 메뉴만 쏙 뽑아서 자동으로 전송합니다.
...
```mermaid
graph TD
    A["🐳 Docker Container 24/7"] --> B["⏰ Python Schedule (06:00 KST)"]
    B --> C["🌐 생협 사이트 크롤링"]
    C --> D{"🔍 오늘 요일 메뉴 추출"}
...
> **면책 조항 (Disclaimer):**
> 본 프로젝트는 개인적인 용도로 개발되었으며, 경북대학교 및 생활협동조합과는 공식적인 관련이 없습니다. 생협 사이트의 개편이나 정보 제공 방식의 변화에 따라 봇의 동작이 예기치 않게 중단될 수 있으며, 개발자는 이로 인한 문제에 대해 책임지지 않습니다.
